...
# CHANGES:
# - Added flag to remove true labels from data value evaluator
# - Added option to predict data values for not training data
...
class Dvrl(object):
    ...
    Attributes:
      ...
      include_labels: flag for including labels as input to dve.

  """

    def __init__(self, x_train, y_train, x_valid, y_valid,
                 problem, pred_model, parameters, checkpoint_file_name, flags,
                 include_labels=True, include_predictions=False):
        """Initializes DVRL."""

        assert not np.logical_and(
            include_labels, include_predictions), "Can not be true at the same time!"
        ...
        self.batch_size = np.min(
            [parameters['batch_size'], len(x_train[:, 0])])
        ...
        if isinstance(self.data_dim, int):
            _dims.append(self.data_dim)
        else:
            _dims.extend(list(self.data_dim))

        self.x_input = tf.placeholder(tf.float32, _dims)
    ...
    """
        ...
        name = 'data_value_estimator' if self.include_labels else 'data_value_estimator_wo_labels'
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):

            inputs = tf.concat((self.x_input, self.y_input),
                               axis=1) if self.include_labels else self.x_input
            ...
            # Maybe combine with y_hat
            if self.include_labels or self.include_predictions:
                comb_layer = tf.concat((inter_layer, self.y_hat_input), axis=1)
            else:
                comb_layer = inter_layer
    ...
    """
        ...
        dve_loss = (-self.reward_input * prob) + \
            1e3 * (tf.maximum(tf.reduce_mean(est_data_value) - self.threshold, 0) +
                   tf.maximum((1 - self.threshold) - tf.reduce_mean(est_data_value),
                              0)
                   )
        ...
        if perf_metric == 'auc':
            ...
            valid_perf = metrics.accuracy_score(
        ...
        for _ in tqdm.tqdm(range(self.outer_iterations)):
            ...
            # Batch selection
            batch_idx = \
                np.random.permutation(len(self.x_train[:, 0]))[
                    :self.batch_size]
            ...
            if self.include_predictions:
                y_hat_batch = y_train_valid_pred[batch_idx]
            ...
            # Reward computation
            if perf_metric == 'auc':
                dvrl_perf = metrics.roc_auc_score(
                ...
                dvrl_perf = metrics.mean_squared_error(
                    self.y_valid, y_valid_hat)
        ...
        final_data_values = [sess.run(est_data_value, feed_dict={self.x_input: [self.x_train[i]],
                                                                 self.y_input: [self.y_train_onehot[i]],
                                                                 self.y_hat_input: [y_pred_diff[i]]})[:, 0]
                             for i in range(self.x_train.shape[0])]

        final_data_value = np.array(final_data_values).ravel()
    ...
    def predict_dv(self, x_train, y_train):
        ...
        if self.include_predictions:
            y_train_hat = y_train_valid_pred
        ...
        final_data_values = [sess.run(est_data_value, feed_dict={self.x_input: [x_train[i]],
                                                                 self.y_input: [y_train_onehot[i]],
                                                                 self.y_hat_input: [y_train_hat[i]]})[:, 0]
