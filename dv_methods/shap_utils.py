"""Original code: https://github.com/amiratag/DataShapley"""
...
def convergence_plots(marginals):
    ...
    for i, idx in enumerate(np.arange(min(25, marginals.shape[-1]))):
        plt.subplot(5, 5, i + 1)
        plt.plot(np.cumsum(marginals[:, idx]) /
                 np.arange(1, len(marginals) + 1))
...
def return_model(mode, **kwargs):
    if inspect.isclass(mode):
        assert getattr(
            mode, 'fit', None) is not None, 'Custom model family should have a fit() method'
        model = mode(**kwargs)
    elif mode == 'logistic':
        ...
        model = RandomForestClassifier(
            n_estimators=n_estimators, random_state=666)
    elif mode == 'GB':
        n_estimators = kwargs.get('n_estimators', 50)
...
def label_generator(problem, X, param, difficulty=1, beta=None, important=None):
    ...
    def funct_init(inp): return np.sum(
        beta * generate_features(inp[:, important_dims], difficulty), -1)
    ...
    def funct(x): return (np.sum(beta * generate_features(
...
def one_iteration(clf, X, y, X_test, y_test, mean_score, tol=0.0, c=None, metric='accuracy'):
    ...
    idxs, marginal_contribs = np.random.permutation(
        len(c.keys())), np.zeros(len(X))
    new_score = np.max(np.bincount(y)) * 1. / \
        len(y) if np.mean(y // 1 == y / 1) == 1 else 0.
    start = 0
    if start:
        X_batch, y_batch = \
            np.concatenate([X[c[idx]] for idx in idxs[:start]]), np.concatenate(
                [y[c[idx]] for idx in idxs[:start]])
    else:
        X_batch, y_batch = np.zeros(
            (0,) + tuple(X.shape[1:])), np.zeros(0).astype(int)
    for n, idx in enumerate(idxs[start:]):
        ...
        old_score = new_score
        X_batch, y_batch = np.concatenate(
            [X_batch, X[c[idx]]]), np.concatenate([y_batch, y[c[idx]]])
...
def marginals(clf, X, y, X_test, y_test, c=None, tol=0., trials=3000, mean_score=None, metric='accuracy'):
    ...
    for trial in range(trials):
        ...
        marginal, idx = one_iteration(
            clf, X, y, X_test, y_test, mean_score, tol=tol, c=c, metric=metric)
...
def shapley(mode, X, y, X_test, y_test, stop=None, tol=0., trials=3000, **kwargs):
    try:
        ...
        for i in range(trials):
            print(i)
            output = one_pass(mode, X, y, X_test, y_test,
                              tol=tol, stop=stop, **kwargs)
...
def early_stopping(marginals, idxs, stopping):
    ...
    for i in range(len(marginals)):
        stopped_marginals[i][idxs[i][:stopping]
                             ] = marginals[i][idxs[i][:stopping]]
...
def error(mem):
    if len(mem) < 100:
        return 1.0
    all_vals = (np.cumsum(mem, 0) /
                np.reshape(np.arange(1, len(mem) + 1), (-1, 1)))[-100:]
    errors = np.mean(
        np.abs(all_vals[-100:] - all_vals[-1:]) / (np.abs(all_vals[-1:]) + 1e-12), -1)
