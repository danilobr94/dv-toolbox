...
Changes:
    - Added option to include base set X_base and y_base
"""
...
import tensorflow as tf
import sys
from dv_methods.shap_utils import *
...
class DShap(object):
    ...
    def __init__(self, X, y, X_test, y_test, num_test, sources=None,
                 ...
                 overwrite=False, X_base=None, y_base=None,
                 **kwargs):
        """
        Args:
            X: Data covariates
            ...
            overwrite: Delete existing data and start computations from 
                scratch
            X_base: Baseline set of data points ...
            y_base:
    ...
    def run(self, save_every, err, tolerance=0.01, g_run=True, loo_run=True):
        ...
        while tmc_run or g_run:
            print("Running iteration ",  i, "error", error(self.mem_tmc))
            i += 1
    ...
    def one_iteration(self, tolerance, sources=None):
        ...
        # TODO: add option to append base set here?
        if self.X_base is not None:
            X_batch = np.copy(self.X_base)
            y_batch = np.copy(self.y_base)
        else:
